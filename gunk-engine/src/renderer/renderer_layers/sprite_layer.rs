...
impl ModelLayer
{
    pub fn new(device: &wgpu::Device, queue: &wgpu::Queue, surface_info: &WgpuSurfaceInfo, camera_bind_layout: &[&wgpu::BindGroupLayout]) -> Self
    {
        ...
        let render_pipeline = create_wgpu_render_pipeline(&device, &render_pipeline_layout, &shader, &surface_info.configuration);
...
impl RendererLayer for ModelLayer
{
    ...
    fn render(&mut self, encoder: &mut wgpu::CommandEncoder, surface_view: &wgpu::TextureView, camera_bind_group: &wgpu::BindGroup) -> Result<(), wgpu::SurfaceError>
    {
