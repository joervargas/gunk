...
use crate::renderer::{
    vulkan_renderer::sp_vulkan::{
        ...
        splunk_vk_img::{
            create_vk_sampler, sp_create_vk_cubemap_image, sp_destroy_vk_img, SpVkImage
        }, 
...
impl VkSimpleSkyBoxLayer
{
        ...
        ) -> Self
    {
        ...
        let texture = sp_create_vk_cubemap_image(vk_ctx, texture_files).map_err(|err| { log_err!(err); panic!("Error creating vk cubemap image\n") }).unwrap();
        ...
        let mut shader_modules: Vec<SpVkShaderModule> = vec![
            SpVkShaderModule::new(&vk_ctx.device, to_shader_path("SimpleSkyBox.vert").as_path()),
            SpVkShaderModule::new(&vk_ctx.device, to_shader_path("SimpleSkyBox.frag").as_path())
        ];
...
impl SpVkLayerDraw for VkSimpleSkyBoxLayer
{
...
impl SpVk3dLayerUpdate for VkSimpleSkyBoxLayer
{
